angular.module('sbAdminApp')
  .controller('ReservationCalendarCtrl', function ($scope, ReservationService, ngDialog, moment, calendarConfig, $ocLazyLoad, $window) {
      var vm = this;
      vm.prepareEvents = function () {
          vm.events = [];
          angular.forEach(vm.reservationsList, function (value, key) {
              var statusColor=ReservationService.getStatusColor(value.Status);
              temp_color = {
                  primary: statusColor,
                  secondary: statusColor
              }

              var reservationDate = new Date(ReservationService.getValueByKey(value.AdditionalProperties, "Date"));
              var reservationTime = ReservationService.getValueByKey(value.AdditionalProperties, "Time");
             var timeArr=reservationTime.split(":");
             reservationDate.setHours(timeArr[0]);
             reservationDate.setMinutes(timeArr[1]);
             var reservationDateEnd = moment(reservationDate).add(25, 'm');

              var temp_event = {
                  title: (value.Reservation_Number).toString(),
                  color: temp_color,
      ...
      var getReservations = function () {
          ReservationService.getReservations([ '2', '3', '4', '5'], ['1', '2', '3', '4', '5']).then(function (res) {

              vm.reservationsList = res.data;
              vm.prepareEvents();
          }, function (err) {
              console.log(err);
          });
